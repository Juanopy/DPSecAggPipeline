...
struct nms_adpcm_state {    /* Log of the step size multiplier. Operated on by codewords. */
    int yl;
...
static void
nms_adpcm_update(struct nms_adpcm_state *s) {    /* Variable names from ITU G.726 spec */
    int a1ul;
    int fa1;
    int i;
...
static int16_t
nms_adpcm_reconstruct_sample(struct nms_adpcm_state *s,
                             uint8_t I) {    /* Variable names from ITU G.726 spec */
    int dqx;
...
*/
static uint8_t
nms_adpcm_encode_sample(struct nms_adpcm_state *s,
                        int16_t sl) {    /* Variable names from ITU G.726 spec */
    int d;
    uint8_t I;
...
static int
nms_adpcm_decode_block(SF_PRIVATE *psf, NMS_ADPCM_PRIVATE *pnms, uint16_t block[],
                       int16_t samples[]) {
    int k;
...
static int
nms_adpcm_encode_block(SF_PRIVATE *psf, NMS_ADPCM_PRIVATE *pnms, int16_t samples[],
                       uint16_t block[]) {
    int k;
    unsigned int rms = 0;
