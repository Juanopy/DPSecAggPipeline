        ...
        switch (c) {
            ...
            case 's' :
                ...
                if (psf->header.indx + 4 + (sf_count_t) size + (sf_count_t) (size & 1) >
                    psf->header.len && psf_bump_header_allocation(psf, 4 + size + (size & 1)))
                    return count;
...
header_seek(SF_PRIVATE *psf, sf_count_t position, int whence) {
    switch (whence) {
        case SEEK_SET :
            ...
            if (position >
                psf->header.len) {    /* Too much header to cache so just seek instead. */
        ...
        case SEEK_CUR :
            ...
            if (psf->header.indx + position >
                psf->header.len) {    /* Need to jump this without caching it. */
...
psf_cues_dup(const void *ptr, size_t datasize) {
    ...
    if (pcues->cue_count <= SF_CUES_COUNT (
            datasize)) {    /* check that passed-in datasize is consistent with cue_count in passed-in SF_CUES struct */
...
void
psf_strlcpy_crlf(char *dest, const char *src, size_t destmax,
                 size_t srcmax) {    /* Must be minus 2 so it can still expand a single trailing '\n' or '\r'. */
