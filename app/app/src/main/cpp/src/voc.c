...
voc_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    */
    ...
    if (subformat == SF_FORMAT_PCM_U8 &&
        psf->sf.channels == 1) {    /* samplerate = 1000000 / (256 - rate_const) ; */
        ...
        psf_binheader_writef(psf, "e1311", BHW1 (VOC_SOUND_DATA),
                             BHW3 ((int) (psf->datalength + 1)), BHW1 (rate_const), BHW1 (0));
    } else if (subformat == SF_FORMAT_PCM_U8 &&
               psf->sf.channels == 2) {    /* sample_rate = 128000000 / (65536 - rate_short) ; */
        rate_const = 65536 - 128000000 / psf->sf.samplerate;

        /* First write the VOC_EXTENDED section
        ** 		marker, length, rate_const and compression
        */
        psf_binheader_writef(psf, "e13211", BHW1 (VOC_EXTENDED), BHW3 (4), BHW2 (rate_const),
                             BHW1 (0), BHW1 (1));
        ...
        */
        ...
        switch (subformat) {
            ...
            case SF_FORMAT_ALAW :
                ...
                length = psf->sf.frames * psf->sf.channels * psf->bytewidth + 12;
                psf_binheader_writef(psf, "e1341124", BHW1 (VOC_EXTENDED_II), BHW3 (length),
                                     BHW4 (psf->sf.samplerate), BHW1 (8), BHW1 (psf->sf.channels),
                                     BHW2 (6), BHW4 (0));
                break;
