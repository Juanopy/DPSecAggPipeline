...
audio_detect(SF_PRIVATE *psf, AUDIO_DETECT *ad, const unsigned char *data, int datalen) {
    ...
    if (ad->endianness == SF_ENDIAN_LITTLE &&
        vote.le_float > (3 * datalen) / 4) {    /* Almost certainly 32 bit floats. */
        return SF_FORMAT_FLOAT;
    };

    if (ad->endianness == SF_ENDIAN_LITTLE && vote.le_int_24_32 > (3 * datalen) /
                                                                  4) {    /* Almost certainly 24 bit data stored in 32 bit ints. */
