...
static int32_t
EncodeStereo(ALAC_ENCODER *p, struct BitBuffer *bitstream, const int32_t *input, uint32_t stride,
...
static int32_t
EncodeStereoEscape(ALAC_ENCODER *p, struct BitBuffer *bitstream, const int32_t *input,
                   uint32_t stride, uint32_t numSamples);

static int32_t
EncodeMono(ALAC_ENCODER *p, struct BitBuffer *bitstream, const int32_t *input, uint32_t stride,
           uint32_t channelIndex, uint32_t numSamples);
...
*/
static int32_t
EncodeStereo(ALAC_ENCODER *p, struct BitBuffer *bitstream, const int32_t *inputBuffer,
             uint32_t stride, uint32_t channelIndex, uint32_t numSamples) {
    BitBuffer workBits;
    ...
    RequireAction ((p->mBitDepth == 16) || (p->mBitDepth == 20) || (p->mBitDepth == 24) ||
                   (p->mBitDepth == 32), return kALAC_ParamError;);
    ...
    for (mixRes = 0; mixRes <= maxRes; mixRes++) {
        ...
        switch (p->mBitDepth) {
            case 16:
                mix16(inputBuffer, stride, p->mMixBufferU, p->mMixBufferV, numSamples / dilate,
                      mixBits, mixRes);
                break;
            case 20:
                mix20(inputBuffer, stride, p->mMixBufferU, p->mMixBufferV, numSamples / dilate,
                      mixBits, mixRes);
                break;
            case 24:
        ...
        status = dyn_comp(&agParams, p->mPredictorU, &workBits, numSamples / dilate, chanBits,
                          &bits1);
        RequireNoErr (status, goto Exit;);

        set_ag_params(&agParams, MB0, (pbFactor * PB0) / 4, KB0, numSamples / dilate,
                      numSamples / dilate, MAX_RUN_DEFAULT);
        status = dyn_comp(&agParams, p->mPredictorV, &workBits, numSamples / dilate, chanBits,
                          &bits2);
        RequireNoErr (status, goto Exit;);
...
*/
static int32_t
EncodeStereoFast(ALAC_ENCODER *p, struct BitBuffer *bitstream, const int32_t *inputBuffer,
                 uint32_t stride, uint32_t channelIndex, uint32_t numSamples) {
...
*/
static int32_t
EncodeStereoEscape(ALAC_ENCODER *p, struct BitBuffer *bitstream, const int32_t *inputBuffer,
                   uint32_t stride, uint32_t numSamples) {
    uint8_t partialFrame;
    uint32_t indx;
...
*/
static int32_t
EncodeMono(ALAC_ENCODER *p, struct BitBuffer *bitstream, const int32_t *inputBuffer,
           uint32_t stride, uint32_t channelIndex, uint32_t numSamples) {
    BitBuffer startBits = *bitstream;            // squirrel away copy of current state in case we need to go back and do an escape packet
    AGParamRec agParams;
    uint32_t bits1;
...
alac_get_magic_cookie(ALAC_ENCODER *p, void *outCookie, uint32_t *ioSize) {
    ...
    if (theConfig.numChannels > 2) {
        theChannelLayout.mChannelLayoutTag = Swap32NtoB (
...
*/
int32_t
alac_encoder_init(ALAC_ENCODER *p, uint32_t samplerate, uint32_t channels, uint32_t format_flags,
                  uint32_t frameSize) {
    int32_t status;

    p->mFrameSize = (frameSize > 0 && frameSize <= ALAC_FRAME_LENGTH) ? frameSize
                                                                      : ALAC_FRAME_LENGTH;
...
*/
void
alac_get_source_format(ALAC_ENCODER *p, const AudioFormatDescription *source,
