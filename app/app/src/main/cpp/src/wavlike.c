...
wavlike_read_fmt_chunk(SF_PRIVATE *psf, int fmtsize) {
    ...
    switch (wav_fmt->format) {
        ...
        case WAVE_FORMAT_EXTENSIBLE :
            ...
            bytesread +=
                    psf_binheader_readf(psf, "224", &(wav_fmt->ext.extrabytes),
            ...
            if (wav_fmt->ext.channelmask == 0)
                ...
                if ((psf->channel_map = calloc(psf->sf.channels, sizeof(psf->channel_map[0]))) ==
                    NULL)
...
wavlike_read_bext_chunk(SF_PRIVATE *psf, uint32_t chunksize) {
    ...
    bytes += psf_binheader_readf(psf, "222", &b->max_true_peak_level, &b->max_momentary_loudness,
                                 &b->max_shortterm_loudness);
    bytes += psf_binheader_readf(psf, "j", 180);

    if (chunksize > WAV_BEXT_MIN_CHUNK_SIZE) {    /* File has coding history data. */
...
wavlike_write_bext_chunk(SF_PRIVATE *psf) {
    ...
    */
    ...
    psf_binheader_writef(psf, "222", BHW2 (b->max_true_peak_level),
                         BHW2 (b->max_momentary_loudness), BHW2 (b->max_shortterm_loudness));
...
wavlike_read_cart_chunk(SF_PRIVATE *psf, uint32_t chunksize) {
    ...
    if (chunksize > WAV_CART_MIN_CHUNK_SIZE) {    /* File has tag text. */
...
wavlike_subchunk_parse(SF_PRIVATE *psf, int chunk, uint32_t chunk_length) {
    ...
    if (chunk_length <= 8) {    /* This case is for broken files generated by PEAK. */
