        ...
        switch (marker) {
            ...
            case VHDR_MARKER :
                ...
                psf_binheader_readf(psf, "E4442114", &(vhdr.oneShotHiSamples),
                                    ...
                                    &(vhdr.samplesPerHiCycle), &(vhdr.samplesPerSec),
                                    &(vhdr.octave), &(vhdr.compression),
                                    &(vhdr.volume));
...
svx_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    psf_binheader_writef(psf, "Emsms", BHWm (NAME_MARKER), BHWs (psf->file.name.c),
                         BHWm (ANNO_MARKER), BHWs (annotation));
