...
typedef struct {    /* libsndfile internal : write a PEAK chunk at the start or end of the file? */
    int peak_loc;
...
typedef struct sf_private_tag {
    ...
    union {    /* Place a double here to encourage double alignment. */
        double d[2];
        char c[16];
    } canary;
...
int
psf_store_read_chunk_str(READ_CHUNKS *pchk, const char *marker, sf_count_t offset, uint32_t len);
