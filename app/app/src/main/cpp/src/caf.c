...
decode_desc_chunk(SF_PRIVATE *psf, const DESC_CHUNK *desc) {
    ...
    if (desc->fmt_id == lpcm_MARKER && desc->fmt_flags & 1) {    /* Floating point data. */
    ...
    if (desc->fmt_id == lpcm_MARKER && (desc->fmt_flags & 1) == 0) {    /* Integer data. */
        if (desc->bits_per_chan == 32 && desc->pkt_bytes == 4 * desc->channels_per_frame) {
            psf->bytewidth = 4;
...
caf_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    if (append_free_block) {    /* Add free chunk so that the actual audio data starts at a multiple 0x1000. */
...
put_key_value(struct put_buffer *buf, const char *key, const char *value) {
    ...
    written = snprintf(buf->s + buf->index, sizeof(buf->s) - buf->index, "%s%c%s%c", key, 0, value,
                       0);
