...
test_nms_adpcm_32(void) {
    ...
    buffer = (int16_t * )
    malloc(sizeof(int16_t) * NMS_SAMPLES_PER_BLOCK);

    print_test_name("Testing nms adpcm 32kbs encoder");

    nms_adpcm_codec_init(&nms, NMS32);
    for (i = 0;
         i * NMS_BLOCK_SHORTS_32 < ARRAY_LEN (test_codes_32); i++) {    /* Unpack the reference */
    ...
    nms_adpcm_codec_init(&nms, NMS32);
    for (i = 0; i * NMS_BLOCK_SHORTS_32 < ARRAY_LEN (test_codes_32); i++) {    /* Unpack the code */
        nms_adpcm_block_unpack_32(&(test_codes_32[i * NMS_BLOCK_SHORTS_32]), buffer, NULL);
        for (j = 0; j < NMS_SAMPLES_PER_BLOCK; j++) {
...
test_nms_adpcm_24(void) {
    ...
    for (i = 0;
         i * NMS_BLOCK_SHORTS_24 < ARRAY_LEN (test_codes_24); i++) {    /* Unpack the reference */
    ...
    nms_adpcm_codec_init(&nms, NMS24);
    for (i = 0; i * NMS_BLOCK_SHORTS_24 < ARRAY_LEN (test_codes_24); i++) {    /* Unpack the code */
        nms_adpcm_block_unpack_24(&test_codes_24[i * NMS_BLOCK_SHORTS_24], buffer, NULL);
        for (j = 0; j < NMS_SAMPLES_PER_BLOCK; j++) {
            sl = nms_adpcm_decode_sample(&nms, buffer[j]);
...
test_nms_adpcm_16(void) {
    ...
    for (i = 0;
         i * NMS_BLOCK_SHORTS_16 < ARRAY_LEN (test_codes_16); i++) {    /* Unpack the reference */
    ...
    nms_adpcm_codec_init(&nms, NMS16);
    for (i = 0; i * NMS_BLOCK_SHORTS_16 < ARRAY_LEN (test_codes_16); i++) {    /* Unpack the code */
        nms_adpcm_block_unpack_16(&test_codes_16[i * NMS_BLOCK_SHORTS_16], buffer, NULL);
        for (j = 0; j < NMS_SAMPLES_PER_BLOCK; j++) {
            sl = nms_adpcm_decode_sample(&nms, buffer[j]);
