...
typedef struct {    /* Warning, this filename is NOT nul terminated. */
    char filename[22];
    char software[20];
    char sample_name[22];
...
xi_write_header(SF_PRIVATE *psf, int UNUSED (calc_length)) {
    ...
    psf_binheader_writef(psf, "b1", BHWv (pxi->filename), BHWz (sizeof(pxi->filename)),
                         BHW1 (0x1A));
    ...
    */
    ...
    for (k = 0; k < sample_count; k++) {
        ...
        if (buffer[2] & 1) {
            ...
            psf->instrument->loops[0].mode = (buffer[2] & 2) ? SF_LOOP_ALTERNATING
                                                             : SF_LOOP_FORWARD;
