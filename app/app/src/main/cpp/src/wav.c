    ...
    while (!done) {
        ...
        switch (marker) {
            ...
            case RIFX_MARKER :
                ...
                if (psf->fileoffset > 0 &&
                    psf->filelength > RIFFsize + 8) {    /* Set file length. */
            ...
            case PEAK_MARKER :
                ...
                if ((error = wavlike_read_peak_chunk(psf, chunk_size)) != 0)
                    return error;
                psf->peak_info->peak_loc = ((parsestage & HAVE_data) == 0) ? SF_PEAK_START
                                                                           : SF_PEAK_END;
                break;
            ...
            case cue_MARKER :
...

                {
                    ...
                    while (cue_count) {
                        if ((thisread = psf_binheader_readf(psf, "e44m444", &id, &position,
                                                            &chunk_id, &chunk_start, &block_start,
                                                            &offset)) == 0)
                            break;
                        bytesread += thisread;
                        ...
                        if (cue_index < 10) /* avoid swamping log buffer with cues */
                            psf_log_printf(psf, "   Cue ID : %2d"
                                                ...
                                                "  Offset : %5d\n",
                                           id, position, chunk_id, chunk_start, block_start,
                                           offset);
...
wav_write_fmt_chunk(SF_PRIVATE *psf) {
    ...
    switch (subformat) {
        ...
        case SF_FORMAT_ULAW :
            ...
            psf_binheader_writef(psf, "222", BHW2 (psf->bytewidth * psf->sf.channels), BHW2 (8),
                                 BHW2 (0));
        ...
        case SF_FORMAT_NMS_ADPCM_32 : {
            ...
            bitwidth = subformat == SF_FORMAT_NMS_ADPCM_16 ? 2 : subformat == SF_FORMAT_NMS_ADPCM_24
                                                                 ? 3 : 4;
            ...
            psf_binheader_writef(psf, "422", BHW4 (bytespersec), BHW2 (blockalign),
                                 BHW2 (bitwidth));
            ...
            if (wpriv->wavex_ambisonic != SF_AMBISONIC_NONE)
				...
				*/
                switch (psf->sf.channels) {
                    ...
                    case 8 :    /* 7.1 */
                        psf_binheader_writef(psf, "4",
                                             BHW4 (0x1 | 0x2 | 0x4 | 0x8 | 0x10 | 0x20 | 0x40 |
                                                   0x80));
                        break;
    ...
    */
    ...
    if (psf->cues != NULL) {
        ...
        psf_binheader_writef(psf, "em44", BHWm (cue_MARKER),
                             BHW4 (4 + psf->cues->cue_count * 6 * 4), BHW4 (psf->cues->cue_count));
    ...
    if (psf->header.indx + 16 < psf->dataoffset) {    /* Add PAD data if necessary. */
