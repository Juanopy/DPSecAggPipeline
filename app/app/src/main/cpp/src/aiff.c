    ...
    */
    while (!done) {
        ...
        switch (marker) {
            case FORM_MARKER :
                ...
                if (psf->fileoffset > 0 &&
                    psf->filelength > FORMsize + 8) {    /* Set file length. */
            ...
            case PEAK_MARKER :
                ...
                psf->peak_info->peak_loc = ((found_chunk & HAVE_SSND) == 0) ? SF_PEAK_START
                                                                            : SF_PEAK_END;
                break;
            ...
            case MARK_MARKER :
                psf_log_printf(psf, " %M : %d\n", marker, chunk_size);
                {
                    ...
                    if (mark_count >
                        2500) /* 2500 is close to the largest number of cues possible because of block sizes */
                    {
                    ...
                    for (n = 0; n < mark_count && bytesread < chunk_size; n++) {
                        ...
                        psf->cues->cue_points[n].fcc_chunk = MAKE_MARKER ('d', 'a', 't',
                                                                          'a'); /* always data */
            ...
            default :
                if (chunk_size >= 0xffff0000) {
                    ...
                    psf_log_printf(psf,
                                   "*** Unknown chunk marker (%X) at position %D with length %u. Exiting parser.\n",
    ...
    if (instr_found && mark_found) {
        ...
        for (ji = 0; ji < psf->instrument->loop_count; ji++) {
            if (ji < ARRAY_LEN (psf->instrument->loops)) {
                psf->instrument->loops[ji].start = marker_to_position(paiff->markstr,
                                                                      psf->instrument->loops[ji].start,
                                                                      mark_count);
                psf->instrument->loops[ji].end = marker_to_position(paiff->markstr,
        ...
        if (psf->cues->cue_count > (uint32_t) (psf->instrument->loop_count * 2)) {
            ...
            for (j = 0; j < psf->cues->cue_count - (uint32_t) (psf->instrument->loop_count *
                                                               2); j++) {    /* This simply copies the information in cues above loop positions and writes it at current count instead */
                psf->cues->cue_points[j].indx = psf->cues->cue_points[j +
                                                                      psf->instrument->loop_count *
                                                                      2].indx;
                psf->cues->cue_points[j].position = psf->cues->cue_points[j +
                                                                          psf->instrument->loop_count *
                                                                          2].position;
                psf->cues->cue_points[j].fcc_chunk = psf->cues->cue_points[j +
                                                                           psf->instrument->loop_count *
                                                                           2].fcc_chunk;
                psf->cues->cue_points[j].chunk_start = psf->cues->cue_points[j +
                                                                             psf->instrument->loop_count *
                                                                             2].chunk_start;
                psf->cues->cue_points[j].block_start = psf->cues->cue_points[j +
                                                                             psf->instrument->loop_count *
                                                                             2].block_start;
                psf->cues->cue_points[j].sample_offset = psf->cues->cue_points[j +
                                                                               psf->instrument->loop_count *
                                                                               2].sample_offset;
                for (str_index = 0; str_index < 256; str_index++)
                    psf->cues->cue_points[j].name[str_index] = psf->cues->cue_points[j +
                                                                                     psf->instrument->loop_count *
                                                                                     2].name[str_index];
            };
            psf->cues->cue_count -= psf->instrument->loop_count * 2;
        } else {    /* All the cues were in fact loop positions so we can actually remove the cues altogether */
            free(psf->cues);
            psf->cues = NULL;
        }
    };
...
aiff_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    psf_binheader_writef(psf, "Em42t42", BHWm (COMM_MARKER), BHW4 (comm_size),
                         BHW2 (psf->sf.channels), BHW4 (comm_frames), BHW2 (bit_width));
    ...
    if (psf->instrument != NULL && psf->cues != NULL) {    /* Huge chunk of code removed here because it had egregious errors that were
		** not detected by either the compiler or the tests. It was found when updating
		** the way psf_binheader_writef works.
		*/
    } else if (psf->instrument == NULL && psf->cues != NULL) {    /* There are cues but no loops */
        uint32_t idx;
    ...
    for (k = 0; k < SF_MAX_STRINGS; k++) {
        ...
        switch (psf->strings.data[k].type) {
            case SF_STR_SOFTWARE :
                ...
                psf_binheader_writef(psf, "Em4mb", BHWm (APPL_MARKER), BHW4 (slen + 4),
                                     BHWm (m3ga_MARKER),
...
aiff_read_basc_chunk(SF_PRIVATE *psf, int datasize) {
    ...
    psf->loop_info->loop_mode = (bc.loopType == basc_TYPE_ONE_SHOT) ? SF_LOOP_NONE
                                                                    : SF_LOOP_FORWARD;
...
aiff_get_chunk_data(SF_PRIVATE *psf, const SF_CHUNK_ITERATOR *iterator, SF_CHUNK_INFO *chunk_info) {
    ...
    psf_fread(chunk_info->data, SF_MIN (chunk_info->datalen, psf->rchunks.chunks[indx].len), 1,
              psf);
