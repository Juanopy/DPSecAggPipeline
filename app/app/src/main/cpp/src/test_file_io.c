...
static void test_write_or_die(SF_PRIVATE *psf, void *data, sf_count_t bytes, sf_count_t items,
                              sf_count_t new_position, int linenum);

static void test_read_or_die(SF_PRIVATE *psf, void *data, sf_count_t bytes, sf_count_t items,
                             sf_count_t new_position, int linenum);

static void test_equal_or_die(int *array1, int *array2, int len, int linenum);

static void
test_seek_or_die(SF_PRIVATE *psf, sf_count_t offset, int whence, sf_count_t new_position,
                 int linenum);
...
file_read_write_test(const char *filename) {
    ...
    */
    ...
    test_write_or_die(psf, data_out, sizeof(data_out[0]), ARRAY_LEN (data_out), sizeof(data_out),
                      __LINE__);

    if ((retval = psf_get_filelen(psf)) != sizeof(data_out)) {
        printf("\n\nLine %d: file length after write is not correct (%"
        PRId64
    ...
    */
    ...
    make_data(data_out, ARRAY_LEN (data_out), 2);
    test_read_or_die(psf, data_in, sizeof(data_in[0]), ARRAY_LEN (data_in), 2 * sizeof(data_in),
                     __LINE__);
    test_equal_or_die(data_out, data_in, ARRAY_LEN (data_out), __LINE__);
    ...
    */
    ...
    test_seek_or_die(psf, -1 * SIGNED_SIZEOF (data_out), SEEK_CUR, (sf_count_t)
    sizeof(data_out), __LINE__);

    test_seek_or_die(psf, SIGNED_SIZEOF (data_out), SEEK_CUR, 2 * SIGNED_SIZEOF (data_out),
                     __LINE__);
    ...
    test_seek_or_die(psf, 2 * SIGNED_SIZEOF (data_out), SEEK_SET, 2 * SIGNED_SIZEOF (data_out),
                     __LINE__);
    make_data(data_out, ARRAY_LEN (data_out), 3);
...
static void
test_write_or_die(SF_PRIVATE *psf, void *data, sf_count_t bytes, sf_count_t items,
                  sf_count_t new_position, int linenum) {
    sf_count_t retval;

    retval = psf_fwrite(data, bytes, items, psf);
    if (retval != items) {
        printf("\n\nLine %d: psf_write() returned %"
        PRId64
        " (should be %"
        PRId64
...
static void
test_read_or_die(SF_PRIVATE *psf, void *data, sf_count_t bytes, sf_count_t items,
                 sf_count_t new_position, int linenum) {
    sf_count_t retval;
...
test_seek_or_die(SF_PRIVATE *psf, sf_count_t offset, int whence, sf_count_t new_position,
    ...
    if (retval != new_position) {
        printf("\n\nLine %d: psf_fseek() failed. New position is %"
        PRId64
        " (should be %"
        PRId64
        ").\n\n",
...
test_tell_or_die(SF_PRIVATE *psf, sf_count_t expected_position, int linenum) {
    ...
    if (retval != expected_position) {
        printf("\n\nLine %d: psf_ftell() failed. Position reported as %"
        PRId64
        " (should be %"
        PRId64
        ").\n\n",
