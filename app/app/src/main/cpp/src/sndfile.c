...
sf_error_number(int errnum) {
    ...
    if (errnum < 0 ||
        errnum > SFE_MAX_ERROR) {    /* This really shouldn't happen in release versions. */
...
sf_seek(SNDFILE *sndfile, sf_count_t offset, int whence) {
	...
	** used with SEEK_SET.
	*/
    switch (whence) {    /* The SEEK_SET behaviour is independant of mode. */
        case SEEK_SET :
...
psf_open_file(SF_PRIVATE *psf, SF_INFO *sfinfo) {
    ...
    if (psf->file.mode == SFM_WRITE || (psf->file.mode == SFM_RDWR && psf->filelength == 0)) {    /* If the file is being opened for write or RDWR and the file is currently
    ...
    } else if ((SF_CONTAINER (psf->sf.format)) !=
               SF_FORMAT_RAW) {    /* If type RAW has not been specified then need to figure out file type. */
    ...
    if (psf->file.mode == SFM_WRITE) {    /* Zero out these fields. */
        sfinfo->frames = 0;
        sfinfo->sections = 0;
        sfinfo->seekable = 0;
    };
