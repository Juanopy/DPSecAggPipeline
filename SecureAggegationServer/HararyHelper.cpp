#include "HararyHelper.h"
	...
	HararyHelper::HararyHelper(int amountUsers, int amountNeighbours) {
		...
		normalNVector = nVector;
	}

	void HararyHelper::printHararyMap() {
		for (int iterator = 0; iterator < n; iterator++) {
			for (int i : hararyMap[std::to_string(iterator)]) {
				std::cout << i << ' ';
			}
			std::cout << std::endl;
		}
	...
	void HararyHelper::printVector(std::vector<int> a) {
		for (int i : a) {
			std::cout << i << ' ';
		}
		std::cout << std::endl;
	...
	void HararyHelper::printNgMap() {
		for (int i = 0; i < n; i++) {
			for (std::string it : neighboursMap[std::to_string(i)]) {
				std::cout << "Der Client: " << i << " hat folgende Nachbarn: " << it << std::endl;
			}
		}
	}
