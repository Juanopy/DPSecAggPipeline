...
//k = amount neigbours
    ...
    std::tuple<int, int> HyperGeometricHelper::computeDegreeAndThreshold(int n, double gamma, double delta, int sigma, int eta) {
        int amountUsers = n;
        double fractionDropOutNodes = delta;

        //r(sucess objects), n(sample size), N(total amount of objects) is the order here
        //in python the order is: N, r, n
        double cdfX;
        double cdfY;
        ...
        for (int k = 1; k < n; k++) {
            ...
            for (int t = 1; t < k; t++) {
                ...
                if (cdfY != 0) {
                    std::cout << "k lautet: " << k << std::endl;
                    std::cout << "t lautet: " << t << std::endl;
                    std::cout << "cdfX lautet: " << cdfX << std::endl;
                    std::cout << "cdfy lautet: " << cdfY << std::endl;
                }
                double first = 1 - cdfX + (pow(double(delta + gamma), double(k / 2)));
                double second = cdfY;
                if (first < pow(2, -sigma) / n) {
                    if (second < (pow(2, -eta) / n)) {
                        std::cout << "Gefundene Werte k: " << k << " und t: " << t << std::endl;
